// 링크 저장 시 URL 본문을 가져와 Claude로 제목/요약/카테고리를 만드는 헬퍼.
// 브라우저가 아니라 서버(Vercel Function)에서만 실행되므로 ANTHROPIC_API_KEY가
// 클라이언트에 노출되지 않는다.
...
export async function analyzeWithClaude(url, pageText) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error('서버에 ANTHROPIC_API_KEY가 설정되어 있지 않아요.');
...
{"title": "간결한 제목 (한글, 30자 이내)", "summary": "핵심 내용 요약 (한글, 2~3문장)", "category": "다음 5개 중 정확히 하나: ${CATEGORIES.join(', ')}", "type": "레퍼런스의 형식 — 다음 3개 중 정확히 하나: ${CONTENT_TYPES.join(', ')} (유튜브/영상 플랫폼 링크는 영상, 핀터레스트/인스타그램 사진·이미지 갤러리나 이미지 파일 링크는 이미지, 그 외 글/기사/제품페이지 등은 글)", "aiInsight": "전문가 관점의 인사이트 (한글, 2~3문장)"}`;
  ...
  if (!res.ok) {
    ...
    throw new Error(`Claude API 호출 실패 (${res.status}): ${errBody.slice(0, 200)}`);
  }
