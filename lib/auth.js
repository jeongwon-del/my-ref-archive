...
//                 ADMIN_EMAILS (관리자 이메일, 쉼표로 구분 — 관리자 페이지에만 필요)

import { createHmac, timingSafeEqual } from "node:crypto";
import { parseCookie, stringifySetCookie } from "cookie";
import { getUserById, normalizeEmail } from "./redis.js";
...
// ADMIN_EMAILS 환경변수가 비어 있으면 "관리자 없음" 으로 보고 아무도 통과시키지 않는다.
...
// requireAuth 와 같은 규약: 통과하면 userId, 막으면 응답을 보내고 null.
export async function requireAdmin(req, res) {
  ...
  if (!user || !getAdminEmails().includes(normalizeEmail(user.email))) {
